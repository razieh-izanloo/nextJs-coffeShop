...
export const LatestProducts = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (
        Math.abs(scrollLeft) + parentCards.clientWidth <
        parentCards.scrollWidth
      ) {
        scrollLeft -= 320;
        parentCards.scrollBy({
          left: scrollLeft,
          behavior: "smooth",
        });
      } else {
        scrollLeft = 0;
        parentCards.scrollLeft = 0;
      }
    }, 5000);
      ...
      <div className="overflow-x-auto scroll-swipe" ref={ref}>
        <div className="flex gap-2 py-4 min-w-max">
          {data?.products.map((item) => (
            ...
            >
              <div className="mx-4 -mt-6 h-40  rounded-xl bg-blue-gray-500 bg-clip-border text-white shadow-lg shadow-blue-gray-500/40 bg-[#779bc1] p-2.5">
              ...
              <div className="p-6 pt-0">
                <Link
                  ...
                  className="rounded-lg bg-[#779bc1] py-3 px-6 text-center align-middle font-sans text-xs font-bold uppercase text-white shadow-md shadow-blue-500/20 transition-all hover:shadow-lg hover:shadow-blue-500/40 focus:opacity-[0.85] focus:shadow-none active:opacity-[0.85] active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                >
                  مشاهده
                </Link>
              </div>
